...
let compile_detailed (source: FlyConfigFile) =
    ...
    let parallelViewFlying =
        ...
        if isNull (box sourceValue) then
            add_issue
                ...
                "parallel_view_flying is missing"
            ...
            | _ ->
                add_issue
                    "parallel_view_flying"
                    "parallel_view_flying.mode is invalid"
                    (fun (config: FlyConfigFile) ->
                        { config with
                            parallel_view_flying = defaults.parallel_view_flying })
    ...
    let positiveChecks: PositiveCheck list =
        [ "base_speed", source.base_speed, (fun (config: FlyConfigFile) -> { config with base_speed = defaults.base_speed });
      "minimum_speed",
      source.minimum_speed,
      (fun (config: FlyConfigFile) -> { config with minimum_speed = defaults.minimum_speed });
      "maximum_speed",
      source.maximum_speed,
      (fun (config: FlyConfigFile) -> { config with maximum_speed = defaults.maximum_speed });
      "speed_step_multiplier",
      source.speed_step_multiplier,
      (fun (config: FlyConfigFile) -> { config with speed_step_multiplier = defaults.speed_step_multiplier });
      "boost_multiplier",
      source.boost_multiplier,
      (fun (config: FlyConfigFile) -> { config with boost_multiplier = defaults.boost_multiplier });
      "slow_multiplier",
      source.slow_multiplier,
      (fun (config: FlyConfigFile) -> { config with slow_multiplier = defaults.slow_multiplier });
      ...
      (fun (config: FlyConfigFile) -> { config with mouse_pivot_multiplier = defaults.mouse_pivot_multiplier });
      "mouse_pan_multiplier",
      source.mouse_pan_multiplier,
      (fun (config: FlyConfigFile) -> { config with mouse_pan_multiplier = defaults.mouse_pan_multiplier });
      "mouse_sensitivity",
      source.mouse_sensitivity,
      (fun (config: FlyConfigFile) -> { config with mouse_sensitivity = defaults.mouse_sensitivity });
      ...
      (fun (config: FlyConfigFile) -> { config with vertical_speed_multiplier = defaults.vertical_speed_multiplier });
    ...
    if source.speed_step_multiplier <= 1. then
        add_issue
            "speed_step_multiplier"
            "speed_step_multiplier must be greater than 1"
            (fun (config: FlyConfigFile) ->
                { config with
    ...
    let derivedChecks: DerivedCheck list =
      ...
      source.maximum_speed * maximumMovementMultiplier * source.vertical_speed_multiplier,
      (fun (config: FlyConfigFile) ->
          { config with
              vertical_speed_multiplier = defaults.vertical_speed_multiplier });
      "parallel_up_down_multiplier",
      "maximum parallel up/down speed",
      source.maximum_speed * maximumMovementMultiplier * source.parallel_up_down_multiplier,
      (fun (config: FlyConfigFile) ->
          { config with
              parallel_up_down_multiplier = defaults.parallel_up_down_multiplier });
      "parallel_zoom_speed_multiplier",
      "maximum parallel zoom speed",
      source.maximum_speed * maximumMovementMultiplier * source.parallel_zoom_speed_multiplier,
      (fun (config: FlyConfigFile) ->
          { config with
              parallel_zoom_speed_multiplier = defaults.parallel_zoom_speed_multiplier });
      "parallel_mouse_sensitivity",
    ...
    enum_value
        "mouse_x_mode"
        typeof<MouseAxisMode>
        (box source.mouse_x_mode)
        (fun (config: FlyConfigFile) -> { config with mouse_x_mode = defaults.mouse_x_mode })

    enum_value
        "mouse_y_mode"
        typeof<MouseAxisMode>
        (box source.mouse_y_mode)
        (fun (config: FlyConfigFile) -> { config with mouse_y_mode = defaults.mouse_y_mode })

    enum_value
        "wheel_speed_mode"
        typeof<MouseWheelSpeedMode>
        (box source.wheel_speed_mode)
        (fun (config: FlyConfigFile) -> { config with wheel_speed_mode = defaults.wheel_speed_mode })

    enum_value
        "right_click_entry_mode"
        typeof<RightClickEntryMode>
        (box source.right_click_entry_mode)
        (fun (config: FlyConfigFile) -> { config with right_click_entry_mode = defaults.right_click_entry_mode })

    enum_value
        "default_flight_mode"
        typeof<DefaultFlightMode>
        (box source.default_flight_mode)
        (fun (config: FlyConfigFile) -> { config with default_flight_mode = defaults.default_flight_mode })
    ...
    let retargetChecks: EnumCheck<RetargetMode> list =
        [ "shift_right_click_retarget",
          ...
          (fun (config: FlyConfigFile) -> { config with middle_mouse_retarget = defaults.middle_mouse_retarget });
          "mouse4_retarget",
          source.mouse4_retarget,
          (fun (config: FlyConfigFile) -> { config with mouse4_retarget = defaults.mouse4_retarget });
          "mouse5_retarget",
          source.mouse5_retarget,
          (fun (config: FlyConfigFile) -> { config with mouse5_retarget = defaults.mouse5_retarget });
          "retarget_on_pivot",
          source.retarget_on_pivot,
          (fun (config: FlyConfigFile) -> { config with retarget_on_pivot = defaults.retarget_on_pivot });
          "retarget_on_pan",
          source.retarget_on_pan,
          (fun (config: FlyConfigFile) -> { config with retarget_on_pan = defaults.retarget_on_pan });
    ...
    let actionChecks: EnumCheck<MouseGestureAction> list =
        [ "mouse4_action",
          ...
          (fun (config: FlyConfigFile) -> { config with mouse4_action = defaults.mouse4_action });
          "mouse5_action",
          source.mouse5_action,
          (fun (config: FlyConfigFile) -> { config with mouse5_action = defaults.mouse5_action });
          "middle_mouse_action",
          source.middle_mouse_action,
          (fun (config: FlyConfigFile) -> { config with middle_mouse_action = defaults.middle_mouse_action });
          ...
          (fun (config: FlyConfigFile) -> { config with alt_right_click_action = defaults.alt_right_click_action });
    ...
    enum_value
        "boost_mode"
        typeof<KeyActivationMode>
        (box source.boost_mode)
        (fun (config: FlyConfigFile) -> { config with boost_mode = defaults.boost_mode })

    enum_value
        "slow_mode"
        typeof<KeyActivationMode>
        (box source.slow_mode)
        (fun (config: FlyConfigFile) -> { config with slow_mode = defaults.slow_mode })

    enum_value
        "viewport_paint_mode"
        typeof<ViewportPaintMode>
        (box source.viewport_paint_mode)
        (fun (config: FlyConfigFile) -> { config with viewport_paint_mode = defaults.viewport_paint_mode })

    let config: FlyConfig =
        { bindings =
            { forward = required "forward" source.forward (fun (config: FlyConfigFile) -> { config with forward = defaults.forward })
              backward =
                required "backward" source.backward (fun (config: FlyConfigFile) ->
                    { config with backward = defaults.backward })
              left = required "left" source.left (fun (config: FlyConfigFile) -> { config with left = defaults.left })
              right = required "right" source.right (fun (config: FlyConfigFile) -> { config with right = defaults.right })
              ...
              key_pivot_left =
                required "key_pivot_left" source.key_pivot_left (fun (config: FlyConfigFile) ->
                    { config with key_pivot_left = defaults.key_pivot_left })
              key_pivot_right =
                required "key_pivot_right" source.key_pivot_right (fun (config: FlyConfigFile) ->
                    { config with key_pivot_right = defaults.key_pivot_right })
              mouse_navigation =
                { pivot =
                    { toggle =
                        optional "pivot_toggle" source.pivot_toggle (fun (config: FlyConfigFile) ->
                            { config with pivot_toggle = defaults.pivot_toggle })
                      hold =
                        optional "pivot_hold" source.pivot_hold (fun (config: FlyConfigFile) ->
                            { config with pivot_hold = defaults.pivot_hold }) }
                  pan =
                    { toggle =
                        optional "pan_toggle" source.pan_toggle (fun (config: FlyConfigFile) ->
                            { config with pan_toggle = defaults.pan_toggle })
                      hold =
                        optional "pan_hold" source.pan_hold (fun (config: FlyConfigFile) ->
                            { config with pan_hold = defaults.pan_hold }) } }
              boost = required "boost" source.boost (fun (config: FlyConfigFile) -> { config with boost = defaults.boost })
              ...
              speed_increase =
                optional "speed_increase" source.speed_increase (fun (config: FlyConfigFile) ->
                    { config with speed_increase = defaults.speed_increase })
              speed_decrease =
                optional "speed_decrease" source.speed_decrease (fun (config: FlyConfigFile) ->
                    { config with speed_decrease = defaults.speed_decrease })
              exit_key =
                required "exit_key" source.exit_key (fun (config: FlyConfigFile) ->
                    { config with exit_key = defaults.exit_key })
              cancel_flight_and_restore =
                required
                    "cancel_flight_and_restore"
                    source.cancel_flight_and_restore
                    (fun (config: FlyConfigFile) ->
                        { config with
              ...
              toggle_projection =
                if ParallelViewFlying.has_allowed_viewports parallelViewFlying then
                    optional "toggle_projection" source.toggle_projection (fun (config: FlyConfigFile) ->
                        { config with toggle_projection = defaults.toggle_projection })
                else
                    None }
          movement =
